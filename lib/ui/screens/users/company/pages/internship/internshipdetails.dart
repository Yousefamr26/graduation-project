...
class InternshipDetailsScreen extends StatelessWidget {
  ...
  const InternshipDetailsScreen({required this.internship, super.key});

  String _calculateDaysRemaining() {
    if (internship.deadline.isEmpty) {
      return 'Not set';
    }

    try {
      DateTime deadline = DateFormat('yyyy-MM-dd').parse(internship.deadline);
  ...
  String _calculateDaysSincePosted() {
    if (internship.postedDate.isEmpty) {
      return 'Unknown';
    }

    try {
      DateTime posted = DateFormat('yyyy-MM-dd').parse(internship.postedDate);
      DateTime now = DateTime.now();
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: _buildStatCard(
                          "Applicants",
                          internship.applicantsCount.toString(),
                          Icons.people,
                          Colors.blue,
                        ),
                      ),
                  ...
                  // Internship Details
                  _buildSectionTitle("Internship Information", Icons.info_outline),
                  SizedBox(height: 12),
                  _buildDetailCard(
                    items: [
                      _DetailItem("Type", internship.type, Icons.work_outline),
                      if (internship.location != null && internship.location!.isNotEmpty)
                        _DetailItem("Location", internship.location!, Icons.place),
                      _DetailItem("Duration", internship.duration, Icons.schedule),
                      if (internship.maxTrainees != null)
                        _DetailItem("Max Trainees", internship.maxTrainees.toString(), Icons.groups),
                      _DetailItem(
                        "Payment",
                        internship.isPaid ? "Paid Internship" : "Unpaid Internship",
                        internship.isPaid ? Icons.attach_money : Icons.money_off,
                      ),
                    ],
                  ),
                  ...
                  if (internship.requirements.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: internship.requirements.asMap().entries.map((entry) {
                          return Padding(
                            padding: EdgeInsets.only(
                                bottom: entry.key == internship.requirements.length - 1 ? 0 : 8),
                            child: Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Container(
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        _buildTimelineRow(
                          "Posted Date",
                          internship.postedDate.isNotEmpty ? internship.postedDate : 'Not set',
                          Icons.upload,
                          Colors.blue,
                        ),
                        SizedBox(height: 12),
                        _buildTimelineRow(
                          ...
                          internship.deadline.isNotEmpty ? internship.deadline : 'Not set',
                          Icons.flag,
                          Colors.red,
                        ),
                      ],
