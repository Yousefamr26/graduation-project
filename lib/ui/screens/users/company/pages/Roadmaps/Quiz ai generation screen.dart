...
class _QuizAIGenerationScreenState extends State<QuizAIGenerationScreen> {
  ...
  Future<void> _generateAndPoll() async {
    ...
    try {
      debugPrint('🤖 [GENERATE] roadmapId=${widget.roadmapId} | type=${widget.quizType} | num=${widget.numQuestions}');
  ...
  List<Map<String, dynamic>> _parseQuestions(Map responseData) {
    try {
      ...
      return rawQuestions.map<Map<String, dynamic>>((q) {
        ...
        // ✅ FIX: trust widget.quizType as primary source of truth
        question['type'] = _detectType(question, widget.quizType);

        debugPrint(
          '🔍 [Q] text="${question['text']}" | type=${question['type']} | options=${(question['options'] as List?)?.length ?? 0}',
        );
  ...
  // ✅ FIX: user's selected quizType is the primary source of truth
  String _detectType(Map<String, dynamic> question, String fallbackType) {
    // Always trust what the user selected first.
    // The API backend often ignores quizType and returns MCQ regardless.
    if (fallbackType == 'TrueFalse') return 'TrueFalse';

    // For MCQ: verify it actually has options
    final options = question['options'];
    if (options is List && options.isNotEmpty) return 'MCQ';

    // Last resort: check correctAnswer value
            ...
            children: [
              const Text('AI GENERATION RESULT',
                  style: TextStyle(
                      ...
                      letterSpacing: 1.2)),
              Text(
                widget.quizType == 'TrueFalse'
                    ? 'True / False Questions'
                    : 'Multiple Choice Questions',
                style: const TextStyle(
                    fontSize: 11, color: _kPrimary, fontWeight: FontWeight.w500),
              ),
            ],
          ),
        ),
  ...
  Widget _buildLoadingState() {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(
          'Generating ${widget.quizType == 'TrueFalse' ? 'True/False' : 'MCQ'} questions...',
          style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _kTextDark),
        ),
  ...
  bool _isTrueCorrect(String correctAnswer, dynamic options) {
    ...
    // حالة 2: حرف A أو رقم 1 (True غالباً هو الأول)
    if (ca == 'a' || ca == '1') return true;

    // حالة 3: الـ option اللي correctAnswer بيشاور عليه يحتوي على "true"
    if (options is List && options.isNotEmpty) {
      // لو correctAnswer حرف زي "A" أو "B"
      if (ca.length == 1 && ca.codeUnitAt(0) >= 97) {
        final idx = ca.codeUnitAt(0) - 97; // a=0, b=1
        if (idx < options.length) {
          return options[idx].toString().toLowerCase().contains('true');
        }
      }
      // لو correctAnswer نص كامل يبدأ بـ "a)" أو "1)"
  ...
  bool _isFalseCorrect(String correctAnswer, dynamic options) {
    ...
    // حالة 2: حرف B أو رقم 2 (False غالباً هو التاني)
    if (ca == 'b' || ca == '2') return true;

    // حالة 3: الـ option اللي correctAnswer بيشاور عليه يحتوي على "false"
  ...
  Widget _buildQuestionCard(int index, Map<String, dynamic> question) {
    ...
    // ✅ FIX: use the type we already set in _detectType (respects widget.quizType)
    final questionType = (question['type'] ?? widget.quizType).toString();
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              // ✅ FIX: isMCQ is now driven purely by the resolved type
              child: isMCQ
                  ? Column(
                children: List.generate((options as List).length, (i) {
                  ...
                  if (opt.isNotEmpty && correctAnswer.isNotEmpty) {
                    if (correctAnswer.length == 1 &&
                        opt.length >= 2 &&
                        opt[1] == ')') {
                      isCorrect = opt[0].toUpperCase() ==
                          correctAnswer.toUpperCase();
                    } else if (opt.toUpperCase() ==
                        correctAnswer.toUpperCase()) {
                      isCorrect = true;
                    } else if (opt
                        .toUpperCase()
                        .startsWith(correctAnswer.toUpperCase())) {
                      isCorrect = true;
                    }
                  }
