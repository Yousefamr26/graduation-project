...
class MyProfile extends StatefulWidget {
  ...
  const MyProfile({super.key, this.showBackButton = true});
...
class _MyProfileState extends State<MyProfile> {
  ...
  Widget build(BuildContext context) {
    ...
    return _GraduateProfileProvider(
        ...
        body: BlocBuilder<ProfileCubit, ProfileState>(
          builder: (context, state) {
            if (state is ProfileLoading || state is ProfileInitial) {
              ...
              final careerReadiness = stats['careerReadinessScore'] ?? 0;
                          ...
                          children: [
                            ...
                            ClipRRect(
                              ...
                              child: LinearProgressIndicator(
                                value: (careerReadiness is num
                                        ? careerReadiness.toDouble()
                                        : 0.0) /
                                    100,
                                minHeight: 8,
...
// ── Internal provider (reuses shared ProfileCubit logic) ──────────────────────
class _GraduateProfileProvider extends StatelessWidget {
  final ProfileCubit? parentCubit;
  final Widget child;

  const _GraduateProfileProvider({
    this.parentCubit,
    required this.child,
  });
...
// ── Alias wrapper for backward-compat navigation ──────────────────────────────
class GraduateMyProfile extends MyProfile {
  const GraduateMyProfile({super.key});
}
